...
# Temporary options
opt <- list()
opt$in_csv <- "/home/jaker/pfalci/201908_telomere_lengths/data/202011_ont_r51/200sw_telomere_ranges.perc.sorted.csv"
opt$out_path <- "/home/jaker/pfalci/201908_telomere_lengths/results/202011_ont_r51"
opt$prefix <- "202011"
opt$threads <- 6
opt$median_values <- "2572,883"
opt$rename_samples <- "ont_wt,ont_r51+"
...
df_sr <- sample(unique(df$r.name), 1000)
df_s <- data.frame()
...
for (et in 5:75) {
  ...
  s.result.df.tmp <- foreach(st=5:75, .combine=rbind) %dopar% {
...
ets.df <- et.df[et.df$s.thresh==70&et.df$s.name==unique(s.result.df$s.name)[1], ]
...
# Quick plots
attach(ets.df)
plot(e.thresh, tel.ln.mean, 
     xlab="End Threshold @ s.thresh 70", ylab="Mean Telomere Length")
lines(lowess(e.thresh, tel.ln.mean), col='red')
detach(ets.df)

attach(sts.df)
plot(s.thresh, r.per.st, 
     xlab="Start Threshold @ e.thresh 50", ylab="Reads per Threshold")
...
# threshold
start.threshold <- round(quantile(st.res.vec)[[2]][1])
