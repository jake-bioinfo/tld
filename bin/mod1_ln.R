...
source('~/rscripts/telo_modules/fxns/modes.R')
source('~/rscripts/telo_modules/fxns/ddply_thresh.R')
...
# Import median read lengths from sequencing pool
#median_1 <- 5845
#median_2 <- 5107
...
# Create a data frame with a set threshold and use that data going forward, 65-45
# and return read range of non telomeric repeats
result.df <- ddply_thresh(df, 60, 60, noCores)
