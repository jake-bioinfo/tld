...
hist <- ggplot(result.df[result.df$norm=="normalized",], 
  ...
  geom_vline(data = result.tel.stats[result.tel.stats$norm=="normalized",],
             aes(xintercept = result.tel.stats
                 [result.tel.stats$norm=="normalized",]$mode.1,
                 color = s.name),
             linetype = "dashed",
             alpha = 1,
             size = 2) +
  #geom_vline(data = result.tel.stats[result.tel.stats$norm=="normalized",],
  #           aes(xintercept = result.tel.stats
  #               [result.tel.stats$norm=="normalized",]$mode.2,
  #               color = s.name),
  #           linetype = "dashed",
  #           alpha = 1,
  #           size = 2) +
...
fig1 <- grid.arrange(title, leg, bar_pl_f, hist_pl_f, nrow = 3, ncol = 2,
                     layout_matrix = lay,
                     widths = c(4, 6), heights = c(1.5, 7, 1.5))

fig1.f <- paste(c(result.path, "/", opt$prefix, "_Figure1.png"), collapse = "")
